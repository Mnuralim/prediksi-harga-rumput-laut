...
export const performMultipleRegression = unstable_cache(
  async function performMultipleRegression(
    ...
    try {
      ...
      if (!activeModel) {
        throw new Error("No active regression model found");
      }
      ...
      const seaweeds = await prisma.seaweedPrices.findMany({
        where: {
          AND: whereConditions,
        },
        orderBy,
      ...
      for (const harvest of seaweeds) {
        // Rumus regresi linear berganda: Y = b0 + b1*X1 + b2*X2
        // Y = hasil panen, X1 = cuaca (numeric), X2 = biaya produksi

        const productionQuantity = harvest.productionQuantity;
        const quality = harvest.quality;
        const demand = harvest.demand;

        const prediction =
          activeModel.intercept +
          activeModel.productionQuantityCoeff * productionQuantity +
          activeModel.qualityCoeff * quality +
          activeModel.demandCoeff * demand;

        const actualValue = harvest.price;
        ...
        regressionData.push({
          date: harvest.date,
          actualSeaweedPrice: actualValue,
