...
export class MultipleLinearRegression {
  ...
  public train(X: number[][], y: number[]): RegressionResult {
    ...
    if (X.length < 4) {
      throw new Error(
        "Minimum 4 samples required for training with 3 variables"
      );
    }

    if (X.some((row) => row.length !== 3)) {
      throw new Error(
        "Each sample must have exactly 3 features: productionQuantity, quality, demand"
      );
    }
    ...
    return {
      ...
      productionQuantityCoeff: this.coefficients[1],
      qualityCoeff: this.coefficients[2],
      demandCoeff: this.coefficients[3],
      predictions,
      metrics,
    };
  }
  ...
  public predictSingle(
    ...
    return (
      this.coefficients[0] +
      this.coefficients[1] * productionQuantity +
      this.coefficients[2] * quality +
      this.coefficients[3] * demand
    );
  }
  ...
  public loadCoefficients(coefficients: number[]): void {
    if (coefficients.length !== 4) {
      throw new Error(
        "Coefficients array must have exactly 4 elements: [intercept, productionQuantityCoeff, qualityCoeff, demandCoeff]"
      );
    }
  ...
  public getCoefficientNames(): string[] {
    return [
      "intercept",
  ...
  public getCoefficientInfo(): { [key: string]: number } {
    if (!this.trained) throw new Error("Model must be trained");
